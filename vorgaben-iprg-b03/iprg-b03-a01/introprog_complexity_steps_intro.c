...
long for_linear(int n, int* befehle)
{
    ...
    long sum = 0; *befehle = *befehle +1; //Definition von sum
    *befehle = *befehle +1; //Definition von i
    for(int i = 1; i <= n; ++i) {
        // Zähle die folgende Zeile als genau ein Befehl!
        sum += get_value_one();
        *befehle = *befehle +1; //Koerper
        *befehle = *befehle +1; // Vergleiche
        *befehle = *befehle +1; // Inkrementierung
    }
    *befehle = *befehle +1; // letzter Vergleich
    return sum;
    *befehle = *befehle +1; // return sum
}
...
long for_kubisch(int n, int* befehle)
{
    ...
    *befehle = *befehle +1; //Definition von i
    for(int i = 1; i <= n; ++i) {
      *befehle = *befehle +1; //Inkrementierung
      *befehle = *befehle +1; //Vergleiche
      *befehle = *befehle +1; //Definition von j
        for(int j = 1; j <= n; ++j) {
          *befehle = *befehle +1; //Inkrementierung
          *befehle = *befehle +1; //Vergleiche
          *befehle = *befehle +1; //Definition von k
            for(int k = 1; k <= n; ++k) {
