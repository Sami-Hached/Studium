...
{
    /* HIER implementieren */
    stack_element* new_element = (stack_element*) calloc (1, sizeof(stack_element));
...
float stack_pop(stack* astack)
{
    ...
    if (astack->top == NULL) {
      ...
      float result = astack->top->value;
      stack_element *hilfe = astack->top;
...
void process(stack* astack, char* token)
{
    ...
    } // else if (astack->top != NULL && astack->top->next != NULL) {
          //new element to save the added sum and then add it to the stack
          float result=0.0;
          if (is_add(token)==1) {
              //calculation of sum
              result = stack_pop(astack) + stack_pop(astack);
